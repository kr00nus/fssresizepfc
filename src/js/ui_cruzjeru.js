...
function updateAll() {
  ...
  // Baseado na relação de espessura (h/p)
  // =========================================
  const ratio_hp = h_sub / p;

  // Interpolação Linear para a Cruz:
  // Ratio 0.05 -> alpha = 22 (Substrato muito fino, confinamento extremo nos gaps)
  // Ratio 0.20 -> alpha = 17 (Substrato mais espesso)
  let alpha = 22 - (ratio_hp - 0.05) * ((22 - 17) / (0.2 - 0.05));

  // Proteção para não sair dos limites físicos do artigo
  alpha = Math.max(17, Math.min(22, alpha));

  // =========================================
  // Permissividade Efetiva Corrigida (Modelo Exponencial Dinâmico)
  // =========================================
  ...
  updateChart(labels, data, limitIndex, f_limit, alpha);
}

// ==========================================
// FUNÇÃO: updateChart()
// ==========================================
function updateChart(labels, data, limitIndex, f_limit, alpha) {
  ...
  let infoHtml = `<strong>fr:</strong> ${frFreq.toFixed(2)} GHz | <strong>BW:</strong> ${bw.toFixed(2)} GHz | <strong style="color:#0056b3;">Fator (α) dinâmico aplicado: ${alpha.toFixed(2)}</strong>`;
  if (limitIndex !== -1)
