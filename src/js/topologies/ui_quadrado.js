...
function updateChart(
  ...
  let infoHtml = `<strong>Ressonância (Corte):</strong> ${isNaN(frFreq) ? "-" : frFreq.toFixed(2)} GHz <br> <strong>Banda (-10 dB):</strong> ${bw} GHz <br> <strong>Fator de Qualidade (Q):</strong> ${qFactor}`;
  if (patchHfssData && patchHfssData.length > 0) {
    let frErrorHtml = "";
